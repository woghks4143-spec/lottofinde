  ...
  return (
    <SafeAreaView style={[styles.root, { backgroundColor: t.bgCanvas }]} edges={['top']}>
      ...
      <ScrollView contentContainerStyle={{ padding: 16, gap: 12, paddingBottom: 24 }}>
        ...
        {/* 분석 대상 hero (라이트/다크 자동 분기, 분석법 비교와 동일 디자인) */}
      ...
      {Array.from({ length: ROWS }, (_, r) => (
        <View key={r} style={{ flexDirection: 'row', gap: 3 }}>
          {Array.from({ length: COLS }, (_, c) => {
            ...
            // 배경 색상: 본번호(빨강) > 보너스(파랑) > 영역만(연한 보라) > 비활성
            ...
            // 테두리: 예상수(inArea) 셀은 보라 보더로 강조 (라이트/다크 모두 visible).
...
const styles = StyleSheet.create({
  ...
  // 회차 이동 버튼 — 둥근 사각형 + Icon.chevLeft로 부드러운 외형 (분석법 비교와 동일)
  navArrow: {
  ...
  // "당첨번호 발표 전" 박스 — 배경/보더는 인라인 (t.bgOnHeroPill/borderOnHero)
  upcomingNumsBox: {
